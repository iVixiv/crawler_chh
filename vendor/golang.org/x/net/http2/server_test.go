...
// The heuristic is that the server connection's server loop must schedule
...
// defer sc.closeAllStreamsOnConnClose) when the serverConn server loop
// ended.
func TestServer_NoCrash_HandlerClose_Then_ClientClose(t *testing.T) {
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		// Now force the server loop to end, via closing the connection.
		st.cc.Close()
		select {
